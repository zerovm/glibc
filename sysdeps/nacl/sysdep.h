...
{
    int ret=0;
    if ( __zcall_chown(path, owner, group) < 0 ){
	ret=-1;
	*err = errno;
    }
    else{
...
{
    int ret=0;
    if ( __zcall_fchown(fd, owner, group) < 0 ){
	ret=-1;
	*err = errno;
    }
    else{
